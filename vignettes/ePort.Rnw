...
It should be noted that data files and the corresponding reports they generate contain student names. At times, it may be necessary to anonymize student names in a data file, so that it and any reports that can be generated from it do not contain confidential information. Deidentifying student names can be quickly accomplished by transferring all data files that you wish to deidentify into a common folder that is otherwise empty. It is important to ensure that no other files are in the common folder.

In this vignette, such an example folder is already set up for you. The data files for both sections of Topic 03 are located in the \texttt{DataFiles/Topic03_Deidentified} folder of the \texttt{extdata} folder.

using the \texttt{getNameList} and \texttt{encodeName} functions.
