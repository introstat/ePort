...
\tableofcontents
\setcounter{footnote}{1} \footnotetext{This \LaTeX\ vignette document is created using the \R function \code{Sweave} on the \R package \pkg{ePort}. It is automatically downloaded with the package and can be accessed with the \R command \code{vignette("ePort")}. This vignette was written by Lindsay Rutter.}  \newpage
...
A tabular summary of student scores is provided in the report, as seen in Figure \ref{fig:output1a}. The report also contains a similar overview of student performance in the form of a histogram, which is not included in this vignette.

\begin{center}
\captionsetup{width=0.65\textwidth}
\fbox{\includegraphics[width=0.65\textwidth]{output1a.png}}
\captionof{figure}{Summary statistics of student scores. The mean score on the assignment was low (only 25\%), and even the top-scoring student did not perform well (48\%).}
\label{fig:output1a}
\end{center}

In addition to understanding how students from this section performed overall for this topic, instructors and course coordinators may wish to determine how students performed across the different learning outcomes. Indeed, the report provides a fluctuation diagram for this purporse, see Figure \ref{fig:output1b}.

\begin{center}
\fbox{\includegraphics[width=0.75\textwidth]{output1b.png}}
\captionsetup{width=0.75\textwidth}
\captionof{figure}{Fluctuation diagram of percentage correct by learning outcome. The area of each square corresponds to the number of students who obtained that score for that learning outcome. Learning outcomes are ordered on the horizontal axis by mean score, with the highest mean score positioned on the left side of the axis. The mean score for each learning outcome is represented with a blue horizontal line. For this section, learning outcome D had the highest score, and learning outcome E had the lowest score. However, for each learning outcome, there remained a large proportion of the total of 50 students who scored poorly, and even did not earn any points at all (0\%).}
\label{fig:output1b}
\end{center}

Additionally, instructors and course coordinators may wish to obtain an overview of how students from this section performed across the different question sets within the learning outcomes. The report does generate such a graphic, which is shown in Figure \ref{fig:output1c}. 
...
\captionof{figure}{Comparison of student performance in this section on each of the 89 possible questions of Topic 06, grouped by question set. If any pair of questions in a given question set have a difference in mean scores of more than 15\%, then both questions are flagged. Question set A contains four questions, with mean scores that ranged from 30.00\% to 52.94\%. Three of these questions (IDs: 2, 3, and 4) were flagged as having a mean score that was at least 15\% different than the mean score of at least one other question in this same question set. In contrast, question set I contained two questions, with mean scores that ranged from 63.33\% to 65.00\%. Hence, neither of these two questions were flagged as demonstrating a concerningly large difference of at least 15\% in difficulty level from within the same question set.}
\label{fig:output2b}
\end{center}
...
If you open the long report we generated ({Stat101hwk\_Topic06\_AB-long.pdf}), then you can view the summary for each of the 89 questions in this assignment. Each question has its summary printed on a separate page, beginning on Page 12 and ending on Page 100 of the report. As we saw in Figure \ref{fig:output2b}, the first question of the assignment had a mean score of 40\%. We can ascertain what types of errors students from this section made on this question by viewing its summary (on Page 12 of the report), which is provided below in Figure \ref{fig:output2c}. 

\begin{center}
\captionsetup{width=\textwidth}
\fbox{\includegraphics[width=\textwidth]{output2c.png}}
\captionof{figure}{A detailed summary of student performance for the first question in the assignment. We see that 10 of the 50 students from this section received this question at random out of a question set that contains four similar questions. Only 4 of these 10 students correctly selected answer choice A. The most frequently incorrect response from students was answer choice B, followed by answer choice C. No student incorrectly responded with answer choice D, and one student did not respond.}
\label{fig:output2c}
\end{center}
...
Please ensure that the variable \texttt{dataList} contains exactly the full pathway to the two data files (Topic06.AB.csv and Topic06.CD.csv). If your variable contains additional pathways, then you have more than just the two necessary input data files in your \texttt{DataFiles/Topic06} folder. If that is the case, then you will need to move these extraneous files elsewhere or hardcode the two full pathways into the \texttt{dataList} variable. 

\subsubsection{Output}

You should be able to examine the report we just generated (Stat101hwk\_Topic06\_crossSection\-short.pdf) in our example \texttt{OutputFiles} folder. One of the most immediate pieces of information that a course coordinator may wish to glean is whether or not there are discrepancies in student performance for a given topic across multiple sections. This could be especially true if there are different instructors and/or different teaching methods being employed across the different sections. In the report we just produced, we can indeed quickly compare how the students performed overall for \texttt{Topic 06} for both sections \texttt{AB} and \texttt{CD}, in a plot that has been copied here in Figure \ref{fig:output3a}.
...
The code above should have generated the long edition of the report (Stat101hwk\_Topic06\_crossSection\-long.pdf). While the short report allowed us to compare how students performed overall between sections \texttt{AB} and \texttt{CD} in \texttt{Topic 06}, the long report will allow us to make more detailed comparisons between the sections at the level of learning outcomes and question sets. For instance, the long report includes comparative information at the level of learning outcomes, as is illustrated in Figure \ref{fig:output4a}.

\begin{center}
\captionsetup{width=0.8\textwidth}
\fbox{\includegraphics[width=0.8\textwidth]{output4a.png}}
\captionof{figure}{Each pair of boxplots compares the student performace between sections AB and CD for each learning outcome. For each learning outcome, the two sections were fairly comparable in their performances, although section CD noticeably outperformed in learning outcome D, and section AB noticeably outperformed for learning outcome B. Both sections separately showed superior performance in learning outcome D than they did for all other learning outcomes.}
\label{fig:output4a}
\end{center}

The long edition of the report breaks it down further by comparing the two sections at the level of each question set. This is demonstrated below in Figure \ref{fig:output4b}.

\begin{center}
\captionsetup{width=0.45\textwidth}
\fbox{\includegraphics[width=0.45\textwidth]{output4b.png}}
\captionof{figure}{Mean percentage scores for each question set by section. The question sets are sorted by decreasing overall mean scores. Both question sets with the highest mean scores are from learning outcome D. We discover that learning outcome A had two question sets (A and B) with relatively high mean scores, but two question sets (C and D) with relatively low mean scores.}
\label{fig:output4b}
\end{center}

The long edition of the report also offers tools that do not explicitly compare the sections of interest, but instead, combines data from all the sections of interest to generate more power in any analysis that we could instead do separately using just the data from one section. For instance, we previously demonstrated certain tools in the individual section reports that can allow instructors to confirm that their students will receive equally-challenging randomly-selected subsets of questions; this was shown in Figure \ref{fig:output2b}.\\
...
\captionof{figure}{Combined student performance from sections \texttt{AB} and \texttt{CD} on each question of \texttt{Topic 06}, grouped by question set. If any pair of questions in a given question set have a difference in mean scores of more than 15\%, then both questions are flagged. Question set A contained four problems, and when we only used student performance information from the 50 students in section \texttt{AB}, three IDs (2, 3, and 4) were flagged as having a mean score that was at least 15\% different than the mean score of another question in the same question set (see Figure \ref{fig:output2b}). However, now that we have used student performance information from all 100 students in both setions \texttt{AB} and \texttt{CD}, only two IDs (2 and 3) remain flagged. In contrast, question set I contained two problems, and neither of these problems were flagged when we only used student performance information from the 50 students in section AB. This remains the case now that we have used student performance information from all 100 students in both sections \texttt{AB} and \texttt{CD}.}
\label{fig:output4c}
\end{center}
...
To generate this type of report, we must be sure to provide a \texttt{dataFolder} variable that contains the pathway to a folder that contains our data files of interest. As usual, it is important to ensure that there are no extraneous files in our folder. Since we want to examine two sections (\texttt{AB} and \texttt{CD}) and two topics (\texttt{03} and \texttt{06}), then we need a folder that contains nothing other than the data files for the four possible combinations of these sections and topics.\\

Indeed, the \texttt{ePort} package contains such an example folder, which he have not yet used in this vignette. It is located in \texttt{extdata/DataFiles/Topic03\_06}. You can examine this folder to confirm that there is nothing but our four necessary input files (\texttt{Topic03.AB.csv}, \texttt{Topic03.CD.csv}, \texttt{Topic06.AB.csv}, and \texttt{Topic06.AB.csv}). We save the pathway to each of these four files to a variable called \texttt{dataList}.\\

For this type of report, we must also prepare a few variables before running the \texttt{makeReport()} function. We prepare these variables using the \texttt{setDir()}, \texttt{mergeData()}, and \texttt{subsetData()} functions. For more information about these functions, please use the \texttt{help()} command.\\

The \texttt{dataTable} variable we create below is a simple data frame that specifies the topic and section for each input data file of interest. The \texttt{mergedData} variable we also create below is a list of data frames that contains the scores that each student from both sections earned on each topic assignment, the number of missed assignments, and the total scores across all students for each section and topic.\\
...
Lastly, we can run our \texttt{makeReport()} function on this section and group of topics. We should specify our unit variable, which contains a default value of 1. In our example, we are indicating that we consider topics \texttt{03} and \texttt{06} to be from Unit 1.\\
...
Upon running this code, you should have created two files into your \texttt{OutputFiles} folder, \texttt{Stat101hwk\_Unit1\_AB.pdf} and \texttt{Stat101hwk\_Unit1\_CD.pdf}. Each of these files compares how students in that section performed for both topics that we defined as comprising Unit 1 (we defined these as Topics \texttt{03} and \texttt{06}).

\subsubsection{Output}

An instructor might want to know how their section of students is performing throughout the course duration, and in partiular, if there are any topics for which their students perform well or poorly. We will examine one of the two reports we just generated (\texttt{Stat101hwk\_Unit1\_AB.pdf}) to discover the types of tools that instructors could use in such situations. The report begins with a quick summary of how students performed for this section (\texttt{AB}) for both \texttt{Topic 03} and \texttt{Topic 06}, as is provided in Figure \ref{fig:output5a}.
...
We can also compare how this section of students performed between these two topics, not only at an overall level, but also at the individual student level, see Figure \ref{fig:output5b}.
...
\captionof{figure}{Heat map of the student scores in this section for both topics of interest . Some students (such as w.introstat64) performed poorly in both topics. Other students (such as w.introstat40, w.introstat49, and w.introstat70) performed much better in \texttt{Topic 03} than they did in \texttt{Topic 06}.}
\label{fig:output5c}
\end{center}
...
Running the code above should have created three new files, \texttt{Stat101hwk\_Unit1\_crossSection.pdf}, \texttt{Stat101hwk\_Unit1\_AB\_lowScore.txt}, and \texttt{Stat101hwk\_Unit1\_CD\_lowScore.txt}. The first file contains our report that compares all combinations of the two sections and two topics of interest. The latter two .txt files contain lists of the e-mail contacts of students from each respective section who performed poorly consistently across the topics of interest.
...
You may notice that there is only one such .txt file in our \texttt{OutputFiles}, and that it covers students from Section AB. Why do we not have a .txt file for students in Section CD? You can find the answer in the report we generated: Table 6 shows that there were 3 students who performed poorly from Section AB, but no students who performed poorly from Section CD. 
...
If you open either of these files, then you will notice there is a ``Question ID" column and a ``Question" column (which contains Question RsQ). The Question ID refers to the question order that each student completed; in this example assignment, there are 31 Question IDs. Hence, each student received 31 questions, and should have Question IDs for each of these questions consecutively increasing from Question ID 1 to Question ID 31. In contrast, the Question RsQ refers to the absolute ID from \hyperref[sec:ResSection]{Respondus}. In this assignment, in total, there were 86 questions in Respondus, from which each student received their randomly-selected subset of 31 questions Hence, the Question RsQ should range from 1 to 86. \\
...
Upon running the above code, if you return to the \texttt{Topic03\_Split} folder, then you will see that four new files have been created, \texttt{Topic03.AB.part1.csv}, \texttt{Topic03.AB.part2.csv}, \texttt{Topic03.CD.part1.csv}, and \texttt{Topic03.CD.part2.csv}. You can verify that the \texttt{part1} files contain the first 9 Question IDs, while the \texttt{part2} files contain the last 22 Question IDs.
...
Next, we will rerun the \hyperref[sec:oneTopicSectionShort]{short version of the report on one section and one topic}. Here, we are specifying three boolean default parameters to values of \texttt{TRUE}, the \texttt{className} default parameter to a value of "Math123", and the \texttt{lowScore} default parameter to a value of 30.
...
If you examine the \texttt{OutputFiles} directory, you should see that we successfully generated the report \texttt{Math123hwk\_Topic06\_AB-short.pdf}. The name of the report file itself, and the title (and any other mention of the class name within the report) have been changed. Since we set the two boolean parameters \texttt{keepText} and \texttt{keepFiles} to \texttt{TRUE}, then we also find the corresponding .tex, .aux, .out, and .log files. Additionally, since we set the boolean paramater \texttt{keepImage} to \texttt{TRUE}, then we have a separate copy of all images that were printed in the report, each saved separately as .pdf files. In this case, there were two images in the report, which were saved as \texttt{LearningObj\_summary-1.pdf} and \texttt{histScore-1.pdf}.\\

We also set the parameter \texttt{lowScore} to a value of \texttt{30}. Because of this, the number of students who received scores that were deemed ``poor performance" was reduced to 35 students. We can see this both from the table in the report that lists low-performing students, and in the .txt file that contains the contacts for these students, \texttt{Math123hwk\_Topic06\_AB\_lowScore.txt}. Note that previously, when we did not changed the default value of \texttt{80} as our threshold for poor performers, we had 50 students who were deemed to have performed poorly (see Figure \ref{fig:output1d}).\\
...
Now, we will run the \hyperref[sec:crossSecU]{report for one unit (group of topics) comparing multiple sections }, only this time altering the default parameters. We specify the \texttt{clasName} parameter to a value of \texttt{Eng444}, and the \texttt{repeatLowScore} parameter to a value of \texttt{80}.
...
Running the code above should have created three new files, \texttt{Eng444hwk\_Unit1\_crossSection.pdf}, \texttt{Eng444hwk\_Unit1\_AB\_lowScore.txt}, and \texttt{Eng444hwk\_Unit1\_CD\_lowScore.txt}. Both in our report summaries and in our .txt files of e-mail contacts of low-performing students, we see that there are now 31 students from \texttt{Section AB} who performed poorly, and 24 students from \texttt{Section CD} who performed poorly. As expected, the number of students who were deemed to have performed poorly has increased from when we ran this same code using the \hyperref[sec:lowPer]{default value of 20}.
