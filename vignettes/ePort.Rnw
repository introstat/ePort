...
Please create a subdirectory called \texttt{OutputFiles} within the \texttt{extdata} directory on your local computer. Later in this vignette, we will save the example reports we create to the \texttt{OutputFiles} subdirectory.
...
Check briefly that your variable \texttt{outPath} is not an emptry string. If it is an empty string, you probably forgot to create the \texttt{OuptutFiles} subdirectory earlier in the vignette (see \hyperref[sec:ResSection]{Respondus}). 
...
Since we wish to generate the type of report that is the short version of one topic for one section, then we can type the number \texttt{1} into the menu console. If we do so, then the report will be generated. Note that if you receive an error: ``Error in file(con, ``w") : cannot open the connection", then you probably forgot to create the \texttt{OuptutFiles} subdirectory earlier in the vignette (see \hyperref[sec:ResSection]{Respondus}).
...
<<eval=FALSE>>=
dataFolder = system.file("inst/extdata/DataFiles/Chapter20_Split", package = "ePort")
dataList = list.files(path = dataFolder, full.names = TRUE)[1:3]
