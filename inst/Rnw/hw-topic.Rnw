...
print(qplot(CrtPct,data=dat,geom='histogram',facets=Hwk~., binwidth=5,xlab='Correct Percentage',fill=Hwk)+ theme(strip.text.y = element_text(size = 20), legend.position = "none",axis.text=element_text(size=20),axis.title=element_text(size=20)))
dev.off();invisible()
...
print(qplot(CrtPct,data=partplotdat,geom='histogram',facets=Hwk~., binwidth=5,xlab='Correct Percentage',fill=Hwk)+scale_fill_hue(h=c(0,120)+(j-1)*120+15)+theme(strip.text.y = element_text(size = 20), legend.position = "none",axis.text=element_text(size=20),axis.title=element_text(size=20)))
dev.off();invisible()
}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
...
print(qplot(Username,data=tmp1,geom='histogram',binwidth=1,xlab='Number of missing homeworks') + theme(axis.text=element_text(size=25), axis.title=element_text(size=25)))
@
