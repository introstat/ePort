...
topic_pmtrx = multiple_comparison(topic_est,as.matrix(topic_estcov),colnames(avg),nrow(score),FALSE)

sec_est=est[1:(nsection-1)+ntopic]
sec_estcov=estcov[1:(nsection-1)+ntopic,1:(nsection-1)+ntopic]
sec_pmtrx=multiple_comparison(sec_est,as.matrix(sec_estcov),rownames(avg),nrow(score),FALSE)
...
Averagely, section %s has the highest correct probability, and %s has the lowest.}
\\end{wrapfigure}\\par\\end{center}',gsub("\\.pdf$","",fname),
...
clorig = cl(merged$dat$CorrectPct[,1:ntopic+2])
clhc2 = clorig[,1:9]
...
crtrorig = crtr(clorig,ntopic)
dfboot = clbootstrap(merged$dat$CorrectPct[,1:ntopic+2],ntopic,nboot=100,txtbar=FALSE)
dfboot2 = melt(dfboot,id.vars=c('cluster','criteria'))

crtr_bw = dfboot[dfboot$criteria=='SSE',-(1:2)]
crtr_wb = dfboot[dfboot$criteria=='Rsquare',-(1:2)]
range_bw = apply(crtr_bw,1,function(x)c(min(x),max(x)))
range_wb = apply(crtr_wb,1,function(x)c(min(x),max(x)))
...
bestcut_bw = if (length(bestcut_bw)) bestcut_bw[1]+1 else which.min(diff(crtrorig[crtrorig$criteria=='SSE','value']))+2
bestcut_wb = if (length(bestcut_wb)) bestcut_wb[1]+1 else which.max(diff(crtrorig[crtrorig$criteria=='Rsquare','value']))+2
...
print(qplot(cluster,value,data=dfboot2,geom='point',alpha=I(0.1)) + 
  facet_grid(criteria~., scales = "free_y") + geom_line(data=crtrorig))
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.98\\linewidth]{%s}
...
This report is generated by Xiaoyue Cheng, Dianne Cook, and Amy Froelich, using R-3.1.2 with packages knitr, xtable and ggplot2.
