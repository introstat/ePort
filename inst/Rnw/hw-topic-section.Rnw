...
print(qplot(Hwk,`50%`,data=sumdat,geom='line',group=Section,fill=Section,
      facets=Section~.,xlab='Topic',ylab='Correct Percentage') +
        theme(strip.text.y=element_text(size=15), axis.text.x = element_text(angle = 90, hjust = 1)) +
...
if (ntopic < 7) {
  ...
  outImage = c(outImage, paste0(outFile,"/",fname))
  print(qplot(CrtPct,data=dat,geom='histogram',facets=Section~Hwk, binwidth=5,xlab='Correct Percentage',fill=Section) + theme101 + theme(strip.text = element_text(size = 20)))
} else {
  pdf(paste0(outFile,"/",fname),width=1.5*nsection, height=ntopic)
  outImage = c(outImage, paste0(outFile,"/",fname))
  print(qplot(CrtPct,data=dat,geom='histogram',facets=Hwk~Section, binwidth=5,xlab='Correct Percentage',fill=Section) + theme101 + theme(strip.text = element_text(size = 20)))
}
{dev.off();invisible()}
...
print(qplot(cluster,value,data=dfboot2,geom='point',alpha=I(0.1)) + 
  facet_grid(criteria~., scales = "free_y") +
    theme(strip.text.y=element_text(size=15), axis.text=element_text(size=15),axis.title=element_text(size=15),legend.text=element_text(size=15),legend.title=element_text(size=15)) +
    geom_line(data=clusterCriteriaorig))
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.98\\linewidth]{%s}
...
print(ggplot(plotData, aes(Cluster, Proportion)) +
        theme(strip.text.x=element_text(size=15), axis.text=element_text(size=15),axis.title=element_text(size=15),legend.text=element_text(size=15),legend.title=element_text(size=15)) +
