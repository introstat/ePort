...
\centerline{\Large\bf Statistics 101 Homework Analysis}


\section{Overview}
The homework outcome file is \Sexpr{basename(Score_filename)}.
In this homework there are \Sexpr{nQuestion} questions.
...
while the full credit for this homework is
...
print(qplot(TotalScore,data=sumry1,geom='histogram',binwidth=1,main='',xlim=c(0,ms+1)))
{dev.off();invisible()}
...
Tables \ref{tab:LearningObj_summary} and Figure \ref{mar:LearningObj_summary} shows the summary of percentage scores by learning objective sets.
Among all the learning objectives,
...
print(xtable(LOsumry,caption='Summary statistics of the learning objectives. They are computed by the mean percentages correct of the questions in the learning objective. The table is sorted from the highest mean to the lowest.',label='tab:LearningObj_summary'),
...
To analyze the students' performance on different learning objectives, we consider the generalized mixed effects model:
\[
g(E[Y_{ij}|u_{j}])= \tau_{i}+u_{j}
\]
where $i=1,...,$\Sexpr{Student_SetScore$ObjectiveSet$nConceptSet} learning objectives;
$j=1,...,$\Sexpr{rows} students. $Y_{ij}$ is the score (scaled in $[0,1]$) of
the $j$th student in the $i$th learning objective. $\tau_i$ is the fixed effect,
which represents the actual level of the learning objective $i$.
...
print(qplot(score,data=Learning_Objectives,geom='histogram',fill=Objective,binwidth=0.02,ylab="count of students",xlab="Scaled score")+theme(axis.text.x=element_text(size=15),axis.text.y=element_text(size=15)))
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.99\\linewidth]{%s}
\\caption{\\label{mar:crtpct_hist}Histogram of the scaled scores by learning objective}
\\end{marginfigure}',sub('\\.pdf','',fname)))
...
It shows that \Sexpr{LOsumryorder[1]} is the best understood learning objective.
...
pdf(paste0("../",fname),width=4,height=4)
print(qplot(value,name,data=fixedcoef_plot,geom=c('point'),size=variable,color=name,xlab="",ylab="")+geom_vline(xintercept=0,color=I(1))+geom_line(size=I(2.5))+scale_colour_hue(h=c(52,320))+theme(legend.position="NA",axis.text.x=element_text(size=15),axis.text.y=element_text(size=15)))
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.99\\linewidth]{%s}
...
print(qplot(Qset,CrtPct,data=output1,geom='boxplot',fill=LearnObj)+ theme(axis.text.x=element_text(size=15),axis.text.y=element_text(size=15)) + labs(x="Question Set",y="Percentage Correct") )
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.99\\linewidth]{%s}
...
\newpage{}
\section{Students}
The percentages correct by learning objective and total for each student, sorted by the highest to lowest total are displayed in Table \ref{tab:LearningObj_data}. And the ranks in percentage are given in Table \ref{tab:LearningObj_rank}. Figure \ref{fig:LearningObj_rank} shows the percent rank of the students.

<<echo=FALSE,results='asis'>>=
CSScore=cbind(Total, CSScore)
CSScorewithSpace=rbind(CSScore[CSScore[,1]>=60,,drop=FALSE],NA,CSScore[CSScore[,1]<60,,drop=FALSE])
SplitCSSore=widetableinLaTeX(CSScorewithSpace,tablecaption='Sorted learning objective sets and total correct percentages',label='tab:LearningObj_data',cols=10)

CSRank = round(100 - apply(CSScore,2,rank,ties.method='max')/nrow(CSScore)*100,2)
PrintCSRank=widetableinLaTeX(CSRank,tablecaption='Rank of the students by the total and learning objective scores. The percentages are the proportion of students in this section who got a higher score in the corresponding column.',label='tab:LearningObj_rank',cols=10)
...
for (j in 1:nQuestion) {
  ...
  if (CountbyQuestion[j]!=0) {
    ...
    if (any(nchar(as.character(tmpoutput$HWanswer[,j+1]))>12)){
      abbrscore=abbr(tmpoutput$HWanswer[,j+1])$abbrv
      glssry=abbr(tmpoutput$HWanswer[,j+1])$glossary
    ...
    pdf(paste0("../",fname2))
    print(qplot(factor(subset(ScorebyQuestion[,j+1], !is.na(ScorebyQuestion[,j+1]))), main=colnames(ScorebyQuestion)[j+1],xlab='Score')+theme(axis.text = element_text(size = rel(2))))
    dev.off()
