...
Table \ref{tab:LearningObj_summary} and Figure \ref{mar:LearningObj_summary} show the summary of percentage scores by learning outcome.
...
print(qplot(variable,value,data=output1,geom='boxplot',fill=variable)+ theme(legend.position = "none", axis.text.x=element_text(size=15), axis.text.y=element_text(size=15))+ labs(x="Learning Outcome", y="Percentage Correct"))
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}
...
Table \ref{tab:QuestionSet_summary} and Figure \ref{mar:QuestionSet_summary} shows the summary statistics of the percentage correct score by question for each question set. Percentage correct scores for questions in the same question set should have small variability.
...
cat(sprintf('\\begin{marginfigure}
\\includegraphics[width=0.99\\linewidth]{%s}
\\caption{\\label{mar:QuestionSet_summary}Side-by-side boxplots of the question correct percentage score for each question set}
...
SplitCSSore=widetableinLaTeX(CSScorewithSpace,tablecaption='Sorted learning outcomes sets and total correct percentages',label='tab:LearningObj_data',cols=10)

CSRank = round(100 - apply(CSScore,2,rank,ties.method='max')/nrow(CSScore)*100,2)
PrintCSRank=widetableinLaTeX(CSRank,tablecaption='Rank of the students by the total and learning outcome scores. The percentages are the proportion of students in this section who got a higher score in the corresponding column.',label='tab:LearningObj_rank',cols=10)
...
%The correct answer is ',gsub('\\$','\\\\$',gsub('\\%','\\\\%',tmpanswer)),'. Code removed from Question output.
<<echo=FALSE,results='asis'>>=
...
for (j in 1:nQuestion) {
  ...
  outscript = paste(ifelse(is.null(img),"",sprintf("\\vspace{-%dcm}",vspace)),'\\pdfbookmark[2]{',answerkey[[2]][j,1],'}{',answerkey[[2]][j,1],'} (',j,') Question "',names(CountbyQuestion)[j],'" is given on the right. This question is selected from the question set with frequency ',round(keysheet$X..in.Set[1]/keysheet$FromQues[1], 2), '. The question was administered to, ', CountbyQuestion[j],' out of a total of', rows, ' students. The average score is ',round(mean(ScorebyQuestion[,(j+1)],na.rm=TRUE), 2),' out of ',keysheet$QFullPt[1],'.\n\n (Back to the question summary Table \\ref{tab:summary_question}.)',sep="")
  cat(outscript)
