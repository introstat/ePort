...
print(ggplot2::qplot(Score,data=sectionscore,geom='histogram',facets=Section~., binwidth=1, fill=Section)+ ggplot2::theme(legend.position = "none",axis.text.x=ggplot2::element_text(size=15),axis.text.y=ggplot2::element_text(size=15)) )
{dev.off();invisible()}
...
print(ggplot2::qplot(Section,Score,data=sectionscore,geom='boxplot',fill=Section)+ggplot2::theme(legend.position = "none",axis.text.x=ggplot2::element_text(size=15),axis.text.y=ggplot2::element_text(size=15)) )
{dev.off();invisible()}
...
Figure \ref{mar:LOpct} displays the percentage correct of each question by section. The graph is facetted by the learning outcomes. In addition, the order of learning outcomes and sections are rearranged by their average percentages correct.
...
pdf(paste0("../",fname),width=4,height=18)
print(ggplot2::qplot(section,Percent,data=b,geom='point',facets=LearningObj~.,col=Section,size=I(8),alpha=I(0.3)) + ggplot2::theme(legend.position = "none", axis.text.x=ggplot2::element_text(size=15), axis.text.y=ggplot2::element_text(size=15)) + ggplot2::coord_flip() )
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
\\caption{\\label{mar:LOpct}Scatterplot of correct percentage by learning outcomes. The sections are sorted by their overall mean scores, and the learning outcomes are ordered by the mean correct percentage.}\\end{marginfigure}',sub('\\.pdf','',fname)))
...
print(ggplot2::qplot(LearningObj, Percent, data=b, geom='boxplot',fill=Section) + ggplot2::labs(x="Learning Outcome", y="Percentage Correct") + ggplot2::theme(axis.text.x=ggplot2::element_text(size=15), axis.text.y=ggplot2::element_text(size=15)) )
{dev.off();invisible()}
...
# f3 = glmer(Score~Objective+Section+(1|student),data=Learning_Objectives, family=binomial(link="logit"))
 f4 = lme4::glmer(Score~Objective*Section+(1+Section|student),data=Learning_Objectives, family=binomial(link="logit"),weights=FullPoints,nAGQ=0)
...
The question set is a set of questions which are randomly delivered to the students. The questions in each question set should focus on the same learning outcome and be equally difficult.
...
print(ggplot2::qplot(section,Percent,data=b,geom='point',facets=QuestionSet~.,col=Section,size=I(8),alpha=I(0.3)) + ggplot2::theme(legend.position = "none", axis.text.x=ggplot2::element_text(size=15), axis.text.ggplot2::y=element_text(size=15))+ ggplot2::coord_flip() )
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
...
for (j in 1:nQuestion) {
  ...
  if (CountbyQuestion[j]!=0) {
    ...
    pdf(paste0("../",fname2))
    print(ggplot2::qplot(factor(subset(instructor_scores$Qscore[,j], !is.na(instructor_scores$Qscore[,j]))), main=colnames(instructor_scores$Qscore)[j],xlab='Score')+ggplot2::theme(axis.text = ggplot2::element_text(size = rel(2))))
    dev.off()
