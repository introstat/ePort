...
for (j in 1:nQuestion) {
  ...
  if (CountbyQuestion[j]!=0) {
    ...
    print(ggplot2::qplot(abbrscore,main=answerkey$simplekey$Question[j],ggplot2::xlab='Answer')+ ggplot2::theme(plot.title = ggplot2::element_text(size = 25), axis.text = ggplot2::element_text(size = 25), axis.title = ggplot2::element_text(size = 25)) + ggplot2::xlim(names(ord)[order(ord, decreasing=TRUE)])+ggplot2::coord_flip())
    dev.off()
