...
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
\\caption{\\label{mar:LOpct}Scatterplot of correct percentage by learning outcome. The sections are sorted by their overall mean scores, and the learning outcomes are ordered by the mean correct percentage.}\\end{marginfigure}',sub('\\.pdf','',fname)))
