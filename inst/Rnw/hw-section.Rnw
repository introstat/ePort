...
\section{Overview}
The homework results that will be compared are \Sexpr{paste(basename(namelist),collapse=', ')}. Those files record the scores of students who are taught by different instructors. The homework questions examine the understanding of \Sexpr{chpt}.

The histograms of total scores are shown in Figure \ref{mar:hist}, on the right of the page. The range of the scores of this homework is [0,\Sexpr{HWFullPt}]. The summary statistics of the scores are given by Table \ref{tab:summary} and Figure \ref{fig:boxplot-score}. The table is sorted by the mean score of each section. Section \Sexpr{paste(highmean,collapse=',')} \Sexpr{ifelse(length(highmean)==1,"has","have")} the highest mean score, and section \Sexpr{paste(highmedian,collapse=",")} \Sexpr{ifelse(all(highmean==highmedian),"also","")}  \Sexpr{ifelse(length(highmedian)==1,"has","have")} the highest median. Section \Sexpr{paste(highstd,collapse=',')} \Sexpr{ifelse(length(highstd)==1,"has","have")} the largest standard deviation of the scores, while section \Sexpr{paste(lowstd,collapse=',')} \Sexpr{ifelse(length(lowstd)==1,"has","have")} the shortest spread (std.dev).
...
fname=paste0(plotprename,'_overallscore.pdf')
...
print(qplot(Score,data=sectionscore,geom='histogram',facets=Section~., binwidth=1, fill=Section)+ theme(legend.position = "none",axis.text.x=element_text(size=15),axis.text.y=element_text(size=15)) )
{dev.off();invisible()}
...
The learning objectives in this chapter are as follows.
...
Figure \ref{mar:LOpct} displays the percentage correct of each question by section. The graph is facetted by the learning objectives. In addition, the order of learning objectives and sections are rearranged by their average percentages correct.

Table \ref{tab:Oset} and Figure \ref{sbsb-os} give the average percentages correct for each learning objective. The learning objective \Sexpr{rownames(instructor_scores$CsetAvgPct)[which.max(apply(instructor_scores$CsetAvgPct,1,mean))]} has the highest correct percentage, and  \Sexpr{rownames(instructor_scores$CsetAvgPct)[which.min(apply(instructor_scores$CsetAvgPct,1,mean))]} gets the lowest average correct percent.

<<echo=FALSE,results='asis'>>=
fname=paste0(plotprename,'_Scatterplot_LOpct.pdf')
...
pdf(paste0("../",fname),width=4,height=18)
print(qplot(section,Percent,data=b,geom='point',facets=LearningObj~.,col=Section,size=I(8),alpha=I(0.3)) + theme(legend.position = "none", axis.text.x=element_text(size=15), axis.text.y=element_text(size=15)) + coord_flip() )
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
\\caption{\\label{mar:LOpct}Scatterplot of correct percentage by learning objectives. The sections are sorted by their overall mean scores, and the learning objectives are ordered by the mean correct percentage.}\\end{marginfigure}',sub('\\.pdf','',fname)))


print(xtable(instructor_scores$CsetAvgPct[levels(b$LearningObj),levels(b$Section),drop=FALSE],caption='Average percentages correct for each learning objective by section. The learning objectives are sorted by the average percentages correct of all sections, from the highest to the lowest.', label='tab:Oset'), floating=FALSE, tabular.environment = "longtable")

fname=paste0(plotprename,'_Sidebyside_Boxplots_LearningObj.pdf')
pdf(paste0("../",fname),width=12,height=6)
print(qplot(LearningObj, Percent, data=b, geom='boxplot',fill=Section) + labs(x="Learning Objective", y="Percentage Correct") + theme(axis.text.x=element_text(size=15), axis.text.y=element_text(size=15)) )
{dev.off();invisible()}
...
To analyze the students' performance by section and learning objectives, 
we consider the generalized linear mixed model:
\[
g(E[Y_{ijk}|u_{jk}])= \mu+\tau_{i}+s_{j}+\tau s_{ij}+u_{jk}
\]
where $i=1,...,$\Sexpr{length(unique(answerkey$simplekey$Objective.Set))} learning objectives; 
$j=1,...,$\Sexpr{length(namelist)} sections; $k=1,...,n_j$ students.
$Y_{ijk}$ is the score (scaled in $[0,1]$) of the $k$th student of 
section $j$ in the $i$th learning objective. $\tau_i$ and $s_{j}$ are the 
fixed effects of learning objective $i$ and section $j$. 
...
of multiple comparison in the learning objectives and the sections. 
...
obj_pmtrx = multiple_comparison(obj_est,obj_estcov,levels(b$LearningObj),nrow(Learning_Objectives))

print(xtable(obj_pmtrx,caption='P-values of the multiple comparison between learning objectives', label='tab:pvalues_obj',digits=4), floating=FALSE, tabular.environment = "longtable")

sec_r=length(namelist)
sec_est=est[1:(sec_r-1)+obj_r-1]
sec_estcov=estcov[1:(sec_r-1)+obj_r-1,1:(sec_r-1)+obj_r-1]
sec_pmtrx=multiple_comparison(sec_est,sec_estcov,rownames(a),nrow(Learning_Objectives))
...
The question set is a set of questions which are randomly delivered to the students. The questions in each question set should focus on the same learning objective and be equally difficult.

The average percentages correct for question sets are shown in Table \ref{tab:Qset-mean} and Figure \ref{mar:Qsetpct}. Among all the \Sexpr{nrow(instructor_scores$QsetAvgPct)} question sets, question set \Sexpr{rownames(instructor_scores$QsetAvgPct)[which.max(apply(instructor_scores$QsetAvgPct,1,mean))]} has the highest correct percentage. \Sexpr{rownames(instructor_scores$QsetAvgPct)[which.min(apply(instructor_scores$QsetAvgPct,1,mean))]} is the hardest question set which gets the lowest average score.

<<echo=FALSE,results='asis'>>=

fname=paste0(plotprename,'_Scatterplot_QSetpct.pdf')
pdf(paste0("../",fname),width=4,height=19)
print(qplot(section,Percent,data=b,geom='point',facets=QuestionSet~.,col=Section,size=I(8),alpha=I(0.3)) + theme(legend.position = "none", axis.text.x=element_text(size=15), axis.text.y=element_text(size=15))+ coord_flip() )
{dev.off();invisible()}
cat(sprintf('\\begin{marginfigure}\\includegraphics[width=0.95\\linewidth]{%s}
\\caption{\\label{mar:Qsetpct}Scatterplot of Correct Percentage by Question Set.}\\end{marginfigure}',sub('\\.pdf','',fname)))
...
print(xtable(tmp1[levels(b$QuestionSet),c('Qset','LO','#Qn','Overall',levels(b$Section))],caption='Average percentages correct for each question set by section. The question sets are sorted by the section means. The second column indicates the corresponding learning objectives.', label='tab:Qset-mean',align=paste("ccc|cc|",paste(rep('c',sec_r),collapse=''),sep='')), floating=FALSE, tabular.environment = "longtable",include.rownames=FALSE)
...
Table \ref{tab:Qset-sd} presents the standard deviation of the question correct rates by question set and section. Question set \Sexpr{rownames(d[d[,3]>1,4:9])[which.min(rowMeans(d[d[,3]>1,4:9]))]} has the smallest standard deviation, while \Sexpr{rownames(d[,4:9])[which.max(rowMeans(d[,4:9]))]} has the largest standard deviation.

<<echo=FALSE,results='asis'>>=
print(xtable(d,caption='Standard deviations of the percentages correct by section. The second column indicates the corresponding learning objectives. The third column gives the number of questions in each question set.', label='tab:Qset-sd',align=paste("ccc|cc|",paste(rep('c',sec_r),collapse=''),sep='')), floating=FALSE, tabular.environment = "longtable",include.rownames=FALSE)
...
for (j in 1:nQuestion) {
  ...
  if (is.null(img <- attr(tmpquestion,"img"))){
    ...
    if (length(img)==1 || all(diff(img)==1)) {
      ...
      sandwichnote = setdiff(min(img):max(img),img)
  ...
  outscript = paste(ifelse(is.null(img),"",sprintf("\\vspace{-%dcm}",vspace)),'\\pdfbookmark[2]{',answerkey[[2]][j,1],'}{',answerkey[[2]][j,1],'} (',j,') Question "',names(CountbyQuestion)[j],'" is given on the right. The correct answer is ',gsub('\\$','\\\\$',gsub('\\%','\\\\%',tmpanswer)),'. This question is selected with frequency ',round(keysheet$X..in.Set[1]/keysheet$FromQues[1], 2), '. And, ', CountbyQuestion[j],' out of ', rows, ' students answered it. The average score is ',round(mean(instructor_scores$Qscore[,j],na.rm=TRUE), 2),' over ',keysheet$QFullPt[1],'.\n\n (Back to the question summary Table \\ref{tab:summary_question}.)',sep="")
  cat(outscript)
  ...
  if (CountbyQuestion[j]!=0) {
    ...
    if (any(nchar(as.character(instructor_scores$Qanswer[,j]))>12)){
      abbrscore=abbr(instructor_scores$Qanswer[,j])$abbrv
		  glssry=abbr(instructor_scores$Qanswer[,j])$glossary
    ...
    pdf(paste0("../",fname1))
    print(qplot(abbrscore,main=answerkey$simplekey$Question[j],xlab='Answer')+xlim(names(ord)[order(ord, decreasing=TRUE)])+coord_flip()+theme(axis.text = element_text(size = rel(2))))
    dev.off()

    pdf(paste0("../",fname2))
    print(qplot(factor(subset(instructor_scores$Qscore[,j], !is.na(instructor_scores$Qscore[,j]))), main=colnames(instructor_scores$Qscore)[j],xlab='Score')+theme(axis.text = element_text(size = rel(2))))
    dev.off()
