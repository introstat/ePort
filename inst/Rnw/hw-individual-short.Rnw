...
\centerline{\Large\bf Statistics 101 Homework Short Report for \Sexpr{gsub("([a-zA-Z])([0-9])","\\1 \\2",gsub("_"," ",plotprename))}}
\vspace{1cm}
...
<<echo=FALSE,results='asis'>>=
if (nrow(Total) > 0){
x.big <- xtable(dfFull, label ='tab:studentsbelow80',caption ='Students whose correct percentages are less than 80\\%.',align = c("rrr|lr|lr|lr"))
...
pdf(paste0("../",fnameO),width=6,height=7) # orig width=6,height=7
