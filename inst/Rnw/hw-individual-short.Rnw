...
print(qplot(TotalScore,data=sumry1,na.rm = T, geom='histogram',fill=factor(TotalScore<ms*.8),binwidth=1,main='',xlim=c(0,ms+1)) + theme(legend.position="none", axis.text = element_text(size=rel(2.8)), axis.title = element_text(size = rel(2.8))) + labs(x="Total Score", y = "Count"))
@
